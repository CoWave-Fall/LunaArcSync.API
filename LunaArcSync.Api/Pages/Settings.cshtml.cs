    ...
    public class SettingsModel : PageModel
    {
        ...
        private void LoadLaunchSettings()
        {
            ...
            if (httpsProfile != null)
            {
                ...
                var urls = applicationUrl.ToString().Split(';');
                foreach (var url in urls)
                {
                    if (url.StartsWith("https://"))
                    {
                        HttpsPort = int.Parse(new Regex(@":(\d+)").Match(url).Groups[1].Value);
                    }
                    else if (url.StartsWith("http://"))
                    {
                        HttpPort = int.Parse(new Regex(@":(\d+)").Match(url).Groups[1].Value);
                    }
                }
            }
        }
        ...
        private void SaveLaunchSettings()
        {
            ...
            if (httpsProfile != null)
            {
                ...
                var urls = currentApplicationUrl.ToString().Split(';').ToList();
